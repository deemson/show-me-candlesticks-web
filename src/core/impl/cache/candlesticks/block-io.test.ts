...
describe("BlockIO", async () => {
  ...
  const store: Store = {
    load: async (_: string, __: Interval, fromBlockNumber: number, toBlockNumber: number): Promise<BlockMap> => {
    ...
    save: async (_: string, __: Interval, blockMap: BlockMap) => {
      for (const [k, v] of blockMap.entries()) {
        storeBlockMap.set(k, v);
      }
    },
  };
  const io = new BlockIO(new FixedBlockSizeIndexer(3), store);
  ...
  test("save 2 contiguous blocks & load them", async () => {
    await io.save(symbol, interval, [1, 2, 3, 4, 5, 6].map(c));
    ...
    const blocks = await io.load(symbol, interval, t(1), t(6));
  ...
  test("save 2 contiguous blocks & load from the middle of them", async () => {
    ...
    const blocks = await io.load(symbol, interval, t(2), t(5));
    expect(dumpCandlestickBlocks(blocks)).toEqual([
  ...
  test("save block 1, load from block 0", async () => {
    await io.save(symbol, interval, [4, 5, 6].map(c));
    expect(dumpBlockMap(storeBlockMap)).toEqual([
      [1, ["1970-01-04T00:00:00.000Z", "1970-01-05T00:00:00.000Z", "1970-01-06T00:00:00.000Z"]],
    ]);
    const blocks = await io.load(symbol, interval, t(1), t(6));
  ...
  test("save blocks 0, 2, 3 & load 0 to 3", async () => {
    await io.save(symbol, interval, [1, 2, 3, 7, 8, 9, 10, 11, 12].map(c));
    expect(dumpBlockMap(storeBlockMap)).toEqual([
      [0, ["1970-01-01T00:00:00.000Z", "1970-01-02T00:00:00.000Z", "1970-01-03T00:00:00.000Z"]],
    ...
    const blocks = await io.load(symbol, interval, t(1), t(12));
    expect(dumpCandlestickBlocks(blocks)).toEqual([
  ...
  test("save 2 contiguous blocks & load from one to weird timestamp somewhere in between", async () => {
    ...
    const blocks = await io.load(symbol, interval, t(1), addToDate({ unit: "hours", amount: 1 }, d(5), 2).getTime());
    expect(dumpCandlestickBlocks(blocks)).toEqual([
      [
        "1970-01-01T00:00:00.000Z",
        "1970-01-02T00:00:00.000Z",
  ...
  test("save block 0 & load 0, 1, 2", async () => {
    await io.save(symbol, interval, [1, 2, 3].map(c));
    expect(dumpBlockMap(storeBlockMap)).toEqual([
      [0, ["1970-01-01T00:00:00.000Z", "1970-01-02T00:00:00.000Z", "1970-01-03T00:00:00.000Z"]],
    ]);
    const blocks = await io.load(symbol, interval, t(1), t(9));
    expect(dumpCandlestickBlocks(blocks)).toEqual([
      ["1970-01-01T00:00:00.000Z", "1970-01-02T00:00:00.000Z", "1970-01-03T00:00:00.000Z"],
    ]);
  });
  test("save incomplete block 0 and complete 1, 2", async () => {
    await io.save(symbol, interval, [2, 3, 4, 5, 6, 7, 8, 9].map(c));
    expect(dumpBlockMap(storeBlockMap)).toEqual([
      [1, ["1970-01-04T00:00:00.000Z", "1970-01-05T00:00:00.000Z", "1970-01-06T00:00:00.000Z"]],
      [2, ["1970-01-07T00:00:00.000Z", "1970-01-08T00:00:00.000Z", "1970-01-09T00:00:00.000Z"]],
    ]);
  ...
  test("save non-contiguous block 0", async () => {
    await expect(async () => {
      await io.save(symbol, interval, [1, 3].map(c));
    }).rejects.toThrow(
      [
  ...
  test("save indexing issue", async () => {
    ...
    const indexer: Indexer = {
      index: (_: Interval, __: number): Index => {
        if (isFirstCall) {
          isFirstCall = false;
    ...
    await expect(async () => {
      await io.save(symbol, interval, [1, 2].map(c));
    }).rejects.toThrow(
      [
